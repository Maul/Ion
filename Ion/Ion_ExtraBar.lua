...
local XBTNIndex, SKINIndex = ION.XBTNIndex, ION.SKINIndex
...
function XBTN:SetAux()
	...
	if (self.vlbtn) then
		...
		if (SKIN) then
			...
			SKIN:Group("Ion", "Vehicle Leave"):AddButton(self.vlbtn, btnData)
...
function XBTN:CreateVehicleLeave(index)
	...
	self.vlbtn.iconframeicon:SetTexture(ION.SpecialActions.vehicle)
...
local function controlOnEvent(self, event, ...)
	...
	if (event == "ADDON_LOADED" and ... == "Ion-PetBar") then
		...
		ION:RegisterBarClass("extrabar", "Extra Action Bar", "Extra Action Button", xbarsGDB, xbarsCDB, XBTNIndex, xbtnsGDB, "CheckButton", "IonActionButtonTemplate", { __index = XBTN }, 1, false, STORAGE, gDef, nil, false)

		ION:RegisterGUIOptions("extrabar", { AUTOHIDE = true,
		                    	             SHOWGRID = true,
		                    	             SNAPTO = true,
		                    	             UPCLICKS = true,
		                    	             DOWNCLICKS = true,
