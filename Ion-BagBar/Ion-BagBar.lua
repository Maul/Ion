--Ion Bag Bar, a World of Warcraft® user interface addon.
...
local defGDB, defCDB = CopyTable(IonBagGDB), CopyTable(IonBagCDB)
...
function ION.IonBackpackButton_OnLoad(self)
...
function ION.IonBackpackButton_OnReceiveDrag(self, button)
...
function ION.IonBackpackButton_OnEnter(self)
...
function ION.IonBackpackButton_OnLeave(self)
	GameTooltip:Hide()
end

function ION.IonBackpackButton_OnEvent(self, event, ...)
...
function ANCHOR:SetData(bar)
	...
	if (bar) then
		...
		if (self.element == IonBackpackButton) then
...
local function controlOnEvent(self, event, ...)
	...
	if (event == "ADDON_LOADED" and ... == "Ion-BagBar") then

		bagElements[1] = IonBackpackButton
		bagElements[2] = Ion______Bag0Slot
		bagElements[3] = Ion______Bag1Slot
		bagElements[4] = Ion______Bag2Slot
		bagElements[5] = Ion______Bag3Slot
		...
		ION:RegisterGUIOptions("bag", { AUTOHIDE = true, SHOWGRID = false, SPELLGLOW = false, SNAPTO = true, DUALSPEC = false, HIDDEN = true, LOCKBAR = false, TOOLTIPS = true }, false, false)
		...
		if (GDB.firstRun) then
			...
			for i=1,#bagElements do
				object = ION:CreateNewObject("bag", i)
			...
			for id,data in pairs(bagbarsGDB) do
				if (data ~= nil) then
					ION:CreateNewBar("bag", id)
				end
			end

			for id,data in pairs(bagbtnsGDB) do
				if (data ~= nil) then
					ION:CreateNewObject("bag", id)
				end
			end
		end
